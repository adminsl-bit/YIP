-- Drop and recreate organizer_leaderboard view with updated manual scoring logic
...
SELECT 
  ...
  p.party_number,
  p.preevent_scores,
  -- Jury average score (null for admin/journalist)
  CASE 
    WHEN EXISTS (
  ...
  -- Jury converted score (null for admin/journalist)
  CASE 
    WHEN EXISTS (
      SELECT 1 FROM user_roles 
      WHERE user_roles.user_id = p.user_id 
  ...
  -- Final total score: For admin/journalist = preevent + organizer_manual_score, For regular = preevent + jury converted
  CASE 
    WHEN EXISTS (
      SELECT 1 FROM user_roles 
      WHERE user_roles.user_id = p.user_id 
