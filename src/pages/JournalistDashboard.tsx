...
type TabId = 'profile' | 'civic-wall' | 'tree' | 'messages' | 'breaking-news';
...
const navItems: { id: TabId; label: string; icon: string; exclusive?: boolean }[] = [
  ...
  { id: 'breaking-news', label: 'Breaking News',    icon: 'newspaper', exclusive: true },
];
...
const JournalistDashboard = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'breaking-news': return <BreakingNewsTabWrapper />;
    ...
    <div className="flex min-h-screen bg-[#F3F4F6] font-body antialiased">
      ...
      <aside className="hidden md:flex flex-col h-screen w-64 fixed z-50 bg-white border-r border-outline-variant py-6 px-4">
        <div className="mb-8 px-2 pt-2">
          ...
          <span className="inline-flex items-center gap-1 mt-2 px-2 py-0.5 rounded-full bg-secondary/10 text-secondary text-[10px] font-black uppercase tracking-widest font-headline">
          ...
          {navItems.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 text-left ${
                activeTab === item.id
                  ? item.exclusive
                    ? 'text-secondary font-bold bg-secondary/5 border-r-4 border-secondary'
            ...
            >
              ...
              {item.exclusive && (
                <span className="text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-full bg-secondary/15 text-secondary font-headline shrink-0">
                  Press
                </span>
              )}
            </button>
        ...
        {navItems.map(item => (
          <button
            ...
            className={`p-2 transition-colors ${
              activeTab === item.id
                ? item.exclusive ? 'text-secondary' : 'text-primary'
...
// Wrapper to give the breaking news tab a proper page heading
const BreakingNewsTabWrapper = () => (
  <div>
    <header className="mb-10">
      <h1 className="text-4xl font-extrabold font-headline tracking-tight text-primary">
        Breaking <span className="text-secondary">News Studio</span>
      </h1>
      <p className="text-[10px] text-on-surface-variant/40 font-black uppercase tracking-[0.4em] mt-3 flex items-center gap-2 font-headline">
        ...
        Live Parliamentary Press Gallery
      </p>
    </header>
