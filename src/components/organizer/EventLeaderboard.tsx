...
export const EventLeaderboard = () => {
  ...
  const handlePromoteOne = async (entry: RankedEntry, destEventId: string) => {
    ...
    if (error) {
      ...
      // Optimistically mark as promoted — keeps the student in the list with
      // green highlight immediately, without a refetch that would remove them
      // (their profiles.event_id has moved so get_event_leaderboard won't return them)
