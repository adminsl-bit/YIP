...
const AWARD_DEFS: AwardDef[] = [
  ...
  { key: 'best_debater',          name: 'Best Debater Award',                        icon: 'forum',           accent: 'primary',   basis: '90-Sec Speech (40%) + Question Hour (60%)', formula: s => sc(s, 'mupi') / 15 * 40 + sc(s, 'question_hour') / 20 * 60, maxScore: 100, constraint: null },
  { key: 'most_persuasive',       name: 'Most Persuasive Policy Advocate',           icon: 'campaign',        accent: 'secondary', basis: 'Political Acumen (50%) + 90-Sec Speech (50%)', formula: s => sc(s, 'political_acumen') / 10 * 50 + sc(s, 'mupi') / 15 * 50, maxScore: 100, constraint: null },
  { key: 'best_research',         name: 'Best Research & Presentation Award',        icon: 'auto_stories',    accent: 'primary',   basis: '90-Sec Speech (40%) + Bill Presentation (30%) + Committee (30%)', formula: s => sc(s, 'mupi') / 15 * 40 + sc(s, 'bill_presentation') / 15 * 30 + sc(s, 'committee') / 15 * 30, maxScore: 100, constraint: null },
  ...
  { key: 'community_impact',      name: 'Community Impact Award',                    icon: 'diversity_3',     accent: 'tertiary',  basis: '90-Sec Speech (40%) + Zero Hour (30%) + Bill Presentation (30%)', formula: s => sc(s, 'mupi') / 15 * 40 + sc(s, 'zero_hour') / 15 * 30 + sc(s, 'bill_presentation') / 15 * 30, maxScore: 100, constraint: null },
  ...
  { key: 'exemplary_decorum',     name: 'Exemplary Parliamentary Decorum Award',     icon: 'verified',        accent: 'primary',   basis: '90-Sec Speech (34%) + Zero Hour (33%) + Bill Presentation (33%)', formula: s => sc(s, 'mupi') / 15 * 34 + sc(s, 'zero_hour') / 15 * 33 + sc(s, 'bill_presentation') / 15 * 33, maxScore: 100, constraint: null },
  { key: 'independent_voice',     name: 'Independent Voice of the House Award',      icon: 'person_raised_hand',accent: 'secondary',basis: '90-Sec Speech + Zero Hour + Question Hour — Non-aligned', formula: s => sc(s, 'mupi') / 15 * 34 + sc(s, 'zero_hour') / 15 * 33 + sc(s, 'question_hour') / 20 * 33, maxScore: 100, constraint: p => p.party_alignment === 'non_aligned', constraintLabel: 'Non-aligned' },
  { key: 'best_constituency_rep', name: 'Best Constituency Representative Award',    icon: 'location_on',     accent: 'tertiary',  basis: '90-Sec Speech (50%) + Question Hour (30%) + Zero Hour (20%)', formula: s => sc(s, 'mupi') / 15 * 50 + sc(s, 'question_hour') / 20 * 30 + sc(s, 'zero_hour') / 15 * 20, maxScore: 100, constraint: null },
];
